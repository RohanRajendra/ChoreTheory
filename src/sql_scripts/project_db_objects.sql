...
-- Add role column to user_house (idempotent — safe to re-run)
ALTER TABLE user_house
  ADD COLUMN IF NOT EXISTS role ENUM('admin', 'member', 'guest') NOT NULL DEFAULT 'member';
UPDATE user_house SET role = 'admin' WHERE is_admin = TRUE AND role = 'member';
...
-- -------------------------------------------------------------
-- get_expense_trend_by_month
-- Returns total expenses per month for a given house.
-- Powers the expense trend line chart.
-- -------------------------------------------------------------
...
-- -------------------------------------------------------------
-- get_top_spenders
-- Returns the top 5 users by total share in a given house.
-- Powers the top spenders bar chart.
-- -------------------------------------------------------------
...
-- -------------------------------------------------------------
-- get_resource_booking_frequency
-- Returns booking count per resource for a given house.
-- Powers the booking frequency bar chart.
-- -------------------------------------------------------------
...
-- -------------------------------------------------------------
-- get_expense_settlement_breakdown
-- Returns count of user_expense rows by payment_status for a house.
-- Powers the settlement status pie chart.
-- -------------------------------------------------------------
...
-- -------------------------------------------------------------
-- get_resource_utilization_by_type
-- Returns total minutes booked and booking count by resource type.
-- Powers the resource utilization grouped bar chart.
-- -------------------------------------------------------------
